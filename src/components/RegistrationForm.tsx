...
export function RegistrationForm({ onSuccess, onPending, selectedEventId, selectedEvent }: RegistrationFormProps) {
  ...
  const [step, setStep] = useState<number>(1); // wizard step: 1 = name+npa, 2 = rest
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${step === 1 ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-500'}`}>1</div>
        <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${step === 2 ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-500'}`}>2</div>
      </div>

      {/* Step 1: Nama lengkap + NPA (required to proceed) */}
      {step === 1 && (
        <div className="space-y-4">
          <div>
            ...
            <label className="text-[10px] font-black uppercase tracking-[0.1em] text-slate-400 ml-1">
              ...
              (Cek NPA: <a href="https://www.idionline.org/organisasi/info/diranggota" target="_blank" rel="noopener noreferrer" className="underline hover:text-emerald-600">idionline.org</a>)
              </span>
            </label>
            <div className="relative mt-2">
              <Hash className={`absolute left-4 top-1/2 -translate-y-1/2 ${errors.npa ? 'text-red-400' : 'text-slate-300'}`} size={16} />
              <input
              type="text"
              placeholder="Input NPA Anda"
              className={`w-full pl-11 pr-4 py-4 bg-slate-50 border rounded-xl focus:ring-1 outline-none transition-all font-medium text-sm ${
                errors.npa ? 'border-red-200 focus:border-red-600 focus:ring-red-600' : 'border-slate-200 focus:border-emerald-600 focus:ring-emerald-600'
              }`}
              value={formData.npa}
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button
              type="button"
              onClick={() => {
                // validate minimal for step 1
                const miniErrors: any = {};
                if (!formData.fullName || formData.fullName.trim().length < 3) miniErrors.fullName = 'Nama lengkap minimal 3 karakter';
                if (!formData.npa || formData.npa.trim().length === 0) miniErrors.npa = 'NPA wajib diisi untuk melanjutkan';
                if (Object.keys(miniErrors).length) {
                  setErrors((prev) => ({ ...prev, ...miniErrors }));
                  return;
                }
                setErrors({});
                setStep(2);
              }}
              className="bg-emerald-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-300 focus:ring-offset-2"
            >
              Selanjutnya
            </button>
          </div>
      ...
      {/* Step 2: Remaining fields and submit */}
      {step === 2 && (
        <>
          ...
          <div className="pt-6 flex items-center justify-between">
            <button 
              type="button"
              onClick={() => setStep(1)}
              className="px-6 py-3 rounded-lg border font-bold bg-white hover:bg-slate-100 hover:border-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-200 focus:ring-offset-2"
            >
              Kembali
            </button>
            <button 
              ...
              type="submit"
              className="ml-4 bg-emerald-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-emerald-300 focus:ring-offset-2"
            >
